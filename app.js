...
async function openUserProfileModal(uid) {
  ...
  // Avatar — clickable to open history if multiple exist
  const avatarEl = $('profile-avatar-view');
  const avatarHistory = u.avatarHistory || (u.photoURL ? [u.photoURL] : []);
  const showAvatar = canSeeAll || !privacy.hideAvatar;
  if (showAvatar) {
  ...
  avatarEl.style.cursor = (showAvatar && avatarHistory.length > 0) ? 'pointer' : 'default';
  avatarEl.onclick = (showAvatar && avatarHistory.length > 0)
    ? () => openAvatarHistory(avatarHistory)
    : null;
  ...
  // Bio
  const showBio = canSeeAll || !privacy.hideBio;
  $('profile-bio-view').textContent = (showBio && u.bio) ? u.bio : '';
  $('profile-bio-view').style.display = (showBio && u.bio) ? '' : 'none';

  // Song player — launches music mini-bar
  const showMusic = canSeeAll || !privacy.hideMusic;
  if (showMusic && u.songURL) {
    ...
    // Clicking the play button launches the music mini-bar
    $('song-play-btn').onclick = () => {
      closeModal('profile-modal');
      openMusicPlayer(u.songURL, u.songName || 'Любимая песня', u.displayName || '');
    };
  } else {
  ...
  if (verifyBtn) {
    if (currentProfile.username === ADMIN_USERNAME && uid !== currentUser.uid) {
      verifyBtn.classList.remove('hidden');
      verifyBtn.textContent = u.isVerified ? '✗ Убрать верификацию' : '✓ Верифицировать';
...
function renderAvatarHistoryViewer() {
  ...
  $('avatar-history-prev').disabled = avatarHistoryIndex === 0;
  $('avatar-history-next').disabled = avatarHistoryIndex === avatarHistoryList.length - 1;
}
...
function openMusicPlayer(url, name, ownerName) {
  if (musicPlayerAudio) {
  ...
  musicPlayerOwner = ownerName;

  musicPlayerAudio = new Audio(url);
  ...
  if (bar) {
    ...
    $('music-bar-title').textContent  = name;
    $('music-bar-artist').textContent = ownerName;
    updateMusicBar();
  }
}
...
function updateMusicBar() {
  ...
  if (!playBtn) return;
  playBtn.innerHTML = musicPlaying
...
function closeMusicBar() {
  if (musicPlayerAudio) { musicPlayerAudio.pause(); musicPlayerAudio = null; }
  musicPlaying = false;
...
async function startCall(type) {
  ...
  callPc.ontrack = e => {
    callRemoteStream.addTrack(e.track);
    $('call-remote-video').srcObject = callRemoteStream;
  ...
  await new Promise(res => {
    if (callPc.iceGatheringState === 'complete') return res();
    callPc.onicegatheringstatechange = () => {
      if (callPc.iceGatheringState === 'complete') res();
    };
    setTimeout(res, 2000);
  });
  ...
  callUnsub = callRef.onSnapshot(async snap => {
    ...
    if (data.answer && callPc.signalingState === 'have-local-offer') {
      await callPc.setRemoteDescription(new RTCSessionDescription(data.answer));
      stopRingtone(); // ← stop ringback tone the moment answer arrives
      $('call-status').textContent = callType === 'video' ? '' : '🎤 Голосовой звонок';
    }

    if (data.answerCandidates?.length && callPc.remoteDescription) {
      for (const c of data.answerCandidates) {
        try { await callPc.addIceCandidate(new RTCIceCandidate(c)); } catch {}
      }
    }
  });
...
async function acceptCall() {
  ...
  const peer = peerSnap.data() || {};
  showCallOverlay(peer, callType, callType === 'video' ? '' : '🎤 Голосовой звонок');
