...
function initEventListeners() {
  ...
  $('chat-header-info-btn').addEventListener('click', () => {
    if (activeChatType === 'dm') openUserProfileModal(activeChatId.split('_').find(id => id !== currentUser.uid));
  });
...
async function renderChatList(docs) {
  ...
  const items = await Promise.all(docs.map(async doc => {
    ...
    preview = chat.lastMessage?.text || (chat.lastMessage?.type === 'image' ? '🖼 Фото' : chat.lastMessage?.type === 'voice' ? '🎤 Голосовое' : chat.lastMessage?.type === 'vidnote' ? '⭕ Видео-кружок' : chat.lastMessage?.type === 'file' ? '📎 Файл' : '');
...
async function renderMessage(msgId, msg) {
  ...
  if (!isOut && activeChatType === 'group') {
    ...
    if (userSnap.exists) senderName = userSnap.data().displayName;
  }

  const bubbleInner = await buildMessageContent(msg, isOut, senderName, msgId);
  row.innerHTML = bubbleInner;
...
async function openUserProfileModal(uid) {
  ...
  if (u.bannerURL) {
    ...
    banner.style.background = `linear-gradient(135deg, ${u.profileColor || '#5288c1'}88, #1a1a2e)`;
  }

  // Avatar
  renderAvatar($('profile-avatar-view'), u.photoURL, u.displayName, u.profileColor);
...
async function openAdminPanel() {
  ...
  const chansSnap = await db.collection('channels').get();
  ...
  usersSnap.forEach(doc => {
    ...
    item.innerHTML = `
      ...
      ${u.isBanned ? '<span class="admin-ban-chip">Забанен</span>' : `<button class="auth-btn danger small" onclick="adminBanByUid('${u.uid}')">Бан</button>`}
    `;
...
async function banUserByUsername(username) {
  ...
  $('admin-ban-msg').className = 'auth-error hidden';
