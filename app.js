...
async function openChannel(channelId) {
  ...
  const subCount = chan?.subscribersCount || chat?.participants?.length || 0;
  ...
  // Members button — visible to owner and admins
  const membersBtn = $('channel-members-btn');
  if (membersBtn) membersBtn.classList.toggle('hidden', !(isOwner || isAdmin));

  // Input area — visible to owner and admins
  $('channel-input-area').classList.toggle('hidden', !(isOwner || isAdmin));
  ...
  messagesUnsub = db.collection('channels').doc(channelId)
    ...
    .onSnapshot(snap => {
      snap.docChanges().forEach(change => {
        if (change.type === 'added') renderChannelPost(change.doc.id, change.doc.data(), chan?.commentsEnabled, isOwner);
...
function renderChannelPost(postId, post, commentsEnabled, isOwner) {
  ...
  const canDelete = isOwner || currentProfile.username === ADMIN_USERNAME;
